...
async function uploadGPTFileChunks(file, sessionId, signal, onProgress) {
    ...
    const check = async response => {
        if (!response.ok) throw Object.assign(new Error(await getErrorMessageFromResponse(response, '文件上传失败')), { status: response.status });
        return response.json();
    };
    const sendPart = async (url, options) => {
        for (let attempt = 0; ; attempt++) {
            try { return await check(await tuoApiFetch(url, options)); }
            catch (error) {
                if (signal.aborted || attempt >= 2 || (error.status && ![409, 429, 500, 502, 503, 504].includes(error.status))) throw error;
                await new Promise((resolve, reject) => {
                    const abort = () => { clearTimeout(timer); reject(signal.reason); };
                    const timer = setTimeout(() => { signal.removeEventListener('abort', abort); resolve(); }, 600 * (attempt + 1));
    ...
    try {
        for (let offset = 0; offset < file.size; offset += start.chunkBytes) {
            ...
            await sendPart(`${route}/chunks?offset=${offset}`, {
        ...
        return await sendPart(`${route}/complete`, { method: 'POST', headers, signal, body: '{}' });
    } catch (error) {
    ...
    try {
        if (modeAtSend === 'image') {
            ...
            for (const doc of documentsToSend) {
                ...
                const saved = await uploadGPTFileChunks(doc.rawFile, session.id, gptAbortController.signal, percent => {
                    const bucket = Math.floor(percent / 10);
                    if (bucket === lastUploadBucket) return;
                    lastUploadBucket = bucket;
                    updateThinkingStep(thinkingObj, percent < 100
                        ? `正在上传附件：${doc.name} · ${percent}%`
                        : `附件已传输，正在保存到云存储：${doc.name}`, 'upload');
                });
        ...
        if (err.name === 'AbortError') {
            ...
            if (partialText) {
                ...
                const isAttachmentError = /准备附件|传送附件|附件.*上传|分块|原附件已保存/i.test(rawErrorMsg);
                const suggestion = isAttachmentError
                    ? '附件传输与模型回答是两个阶段。请保留当前会话；已经保存的附件可在本会话继续使用。'
                    : isFilter
                    ...
                    : isAccountError
                        ...
                        : '可以稍后在当前会话重试；若持续失败，请把错误编号和发生时间提供给管理员。';
